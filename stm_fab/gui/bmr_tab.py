...
class BMRTab:
    ...
    def delete_device_bmr_dialog(self):
        """Dialog to delete BMR records for the current device"""
        if not self.current_device:
            messagebox.showwarning("No Device", "Please select a device first.")
            return
        
        # Get all BMRs for this device
        try:
            bmr_list = self.app.db_ops.list_bmrs_for_device(self.current_device.device_id)
            
            if not bmr_list:
                messagebox.showinfo("No BMRs", "No BMR records found for this device.")
                return
            
            # Create selection dialog
            win = ctk.CTkToplevel(self.tab)
            win.title("Delete BMR Records")
            ...
            ctk.CTkLabel(
                win,
                text=f"BMR Records for {self.current_device.device_name}",
                font=ctk.CTkFont(size=16, weight="bold")
            ).pack(pady=15)
            
            # Scrollable list
            list_frame = ctk.CTkScrollableFrame(win, height=250)
            list_frame.pack(fill="both", expand=True, padx=10, pady=10)
            
            selected_bmr = tk.IntVar(value=-1)
            ...
            for bmr in bmr_list:
                ...
                ctk.CTkRadioButton(
                    frame,
                    text="",
                    variable=selected_bmr,
                    value=bmr.bmr_id
                ).pack(side="left", padx=5)
                
                info_text = f"Batch: {bmr.batch_number}\n"
                info_text += f"Status: {bmr.status}\n"
                info_text += f"Operator: {bmr.operator}\n"
                if bmr.start_date:
                    info_text += f"Started: {bmr.start_date.strftime('%Y-%m-%d %H:%M')}"
            ...
            def delete_selected():
                bmr_id = selected_bmr.get()
                if bmr_id == -1:
                    messagebox.showwarning("No Selection", "Please select a BMR to delete.")
                    return
                
                # Confirm deletion
                bmr = self.app.db_ops.get_bmr_by_id(bmr_id)
                result = messagebox.askyesno(
                    "Confirm Delete",
                    f"Delete BMR '{bmr.batch_number}'?\n\n"
                    "This will permanently delete the BMR and all its steps from the database.",
                    icon='warning'
                )
                
                if result:
                    try:
                        self.app.db_ops.delete_bmr(bmr_id)
                        messagebox.showinfo("Success", "BMR deleted successfully.")
                        
                        # Clear current BMR if it was the one deleted
                        if self._current_bmr_id == bmr_id:
                            self._current_bmr_id = None
                            self.bmr_steps = []
                            self.bmr_metadata = {
                                "batch_number": "",
                        ...
                        messagebox.showerror("Error", f"Failed to delete BMR:\n{e}")
            ...
            messagebox.showerror("Error", f"Failed to load BMRs:\n{e}")
    
    
    def import_bmr_from_json_dialog(self):
        """Import a BMR from JSON file and link to current device"""
        if not self.current_device:
            messagebox.showwarning("No Device", "Please select a device first.")
            return
        ...
        try:
            ...
            from stm_fab.db.models import BatchManufacturingRecord
            existing = self.app.db_ops.session.query(BatchManufacturingRecord)\
                .filter_by(batch_number=batch_number).first()
            
            if existing:
                result = messagebox.askyesnocancel(
                    "BMR Exists",
                    f"BMR with batch number '{batch_number}' already exists in the database.\n\n"
                    f"Yes: Replace existing BMR\n"
                    f"No: Import with a new batch number\n"
                ...
                if result is None:  # Cancel
                    return
                elif result is False:  # No - create new batch number
                    # Prompt for new batch number
                    new_batch = simpledialog.askstring(
                        "New Batch Number",
                        f"Enter a new batch number (original: {batch_number}):",
                        initialvalue=f"{batch_number}_imported"
                    )
                    if not new_batch:
                        return
                    metadata['batch_number'] = new_batch
            ...
            # Import confirmation
            msg = f"Import BMR from JSON?\n\n"
            msg += f"File: {Path(json_path).name}\n"
            msg += f"Batch: {metadata.get('batch_number', 'Unknown')}\n"
            msg += f"Process: {metadata.get('process_type', 'Unknown')}\n"
            msg += f"Steps: {len(steps)}\n"
            msg += f"Device: {self.current_device.device_name}"
            ...
            # Perform import
            try:
                # Save modified data if batch number changed
                if not overwrite and metadata.get('batch_number') != batch_number:
                    temp_path = Path(json_path).parent / f"temp_{Path(json_path).name}"
                    with open(temp_path, 'w', encoding='utf-8') as f:
                        json.dump(data, f, indent=2)
                    import_path = str(temp_path)
                else:
                    import_path = json_path
                
                bmr = self.app.db_ops.import_bmr_from_json(
                    device_id=self.current_device.device_id,
                    json_path=import_path,
                    overwrite=overwrite
                )
                
                # Clean up temp file
                if import_path != json_path:
                    Path(import_path).unlink()
                
                messagebox.showinfo(
                    "Success",
                    f"BMR imported successfully!\n\n"
                    f"Batch: {bmr.batch_number}\n"
                    f"Steps: {len(bmr.bmr_steps)}"
                )
                
                # Ask if user wants to load it
                if messagebox.askyesno("Load BMR", "Load the imported BMR now?"):
                    self._load_bmr_from_database(bmr.bmr_id)
                
            except Exception as e:
                messagebox.showerror("Import Error", f"Failed to import BMR:\n{e}")
