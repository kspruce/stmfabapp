...
class STMFabGUIEnhanced(DeviceManagementMixin):
    ...
    def create_widgets(self):
        ...
        # 6) Fabrication Steps (managed by FabricationStepsTab class)
        ...
        # Note: Fabrication steps tab is created above via FabricationStepsTab class
        self.create_report_tab()
        self.create_thermal_budget_tab()
        self.root.bind("<Control-f>", self._focus_db_search)
    ...
    def _focus_db_search(self, event=None):
        try:
            # switch to DB tab and focus search
            self.tabview.set("💾  Database")
            if hasattr(self, "db_search_entry"):
                self.db_search_entry.focus_set()
                self.db_search_entry.select_range(0, "end")
        except Exception:
            pass
