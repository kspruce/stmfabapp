...
class STMFabGUIEnhanced(DeviceManagementMixin):
    ...
    def create_menu_bar(self):
        """Create menu bar with analysis options"""
        ...
        analysis_menu.add_command(
            label="Analyze Cooldown Curve",
            command=self.analyze_cooldown_from_flash
        )
        analysis_menu.add_command(
            label="Import LabVIEW Process",
            command=self.import_and_show_labview_process
        )
        analysis_menu.add_separator()
        analysis_menu.add_command(
            label="View Thermal Budget",
            command=self.show_thermal_budget_breakdown
        )
        analysis_menu.add_command(
            label="Compare Cooldown Curves",
            command=self.open_cooldown_comparison_tab
        )
        # View menu
    ...
    def create_widgets(self):
        ...
        LabVIEWAnalysisTab(self, self.tabview)
        self.create_cooldown_tab()
        # Add the new cooldown comparison tab
    ...
    def update_thermal_budget_display(self):
        ...
        if budget.warning_threshold > 0:
            progress = min(budget.total_budget / budget.warning_threshold, 1.0)
        else:
            progress = 0
        ...
        self.thermal_label.configure(
            text=f"{budget.total_budget:.2e} / {budget.warning_threshold:.2e} °C·s ({progress*100:.1f}%)",
            text_color=color
        )
    ...
    def show_thermal_budget_breakdown(self):
        ...
        ctk.CTkLabel(
            total_frame,
            text=f"Total Budget: {budget.total_budget:.2e} °C·s",
            font=ctk.CTkFont(size=16, weight="bold")
        ).pack(pady=10)
        ...
        contributions = [
            ('Degas/Outgas', budget.degas_contribution),
            ('Flash Cleaning', budget.flash_contribution),
            ('H-Termination', budget.hterm_contribution),
            ('Incorporation', budget.incorporation_contribution),
            ('Overgrowth', budget.overgrowth_contribution),
            ('Other', budget.other_contribution)
        ]
        
        for process_name, contribution in contributions:
            if contribution > 0:
                percentage = (contribution / budget.total_budget * 100) if budget.total_budget > 0 else 0
                ...
                progress = ctk.CTkProgressBar(row_frame, width=200)
    ...
    def show_quality_check_dialog(self, step_id, step_name):
        ...
        def save_checks():
            ...
            # Show summary
            summary = self.quality_manager.generate_check_report(results)
    ...
    def analyze_cooldown_from_flash(self):
        ...
        try:
            ...
            # Extract cooldown
            cooldown = analyzer.extract_cooldown_curve(temp, current, time)
    ...
    def initialize_device(self):
        ...
        try:
            ...
            self.current_sample = self.db_ops.get_sample_by_name(sample_name)
            if not self.current_sample:
    ...
    def populate_steps_tab(self):
        ...
        # Add step buttons (simplified for this example)
        for i, step in enumerate(FABRICATION_STEPS[:6]):  # First 6 steps
